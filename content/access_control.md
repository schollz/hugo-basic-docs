...
This document describes the access control model for ProjectX.

All such information is maintained by files in the ProjectX name space itself.
...
By user, we mean an account known to the ProjectX Key service, identified by an
e-mail-like name: `ann@example.com`.
Each valid user has a *user* *root* directory held on at least one Directory
server.
Each user proves identity to ProjectX servers,
...
Absent any other information, every item in the user's ProjectX tree is readable
...
ProjectX tree, only the owner may write them, and read access to the `Access`
files themselves is granted by the ProjectX access control mechanisms described
here.
...
`Access` files may name any user or group in the ProjectX system, including
...
Encrypted packings (described in the [ProjectX Security document](/doc/security.md))  in
ProjectX also have the effect of limiting who can read file contents, by only
...
Because of the semantics of I/O in ProjectX, this is always wholesale replacement.
...
ProjectX has no concept of "execute".
...
The user name `all` (case is ignored) means "any authenticated ProjectX user".
The asterisked user name *@example.com means any authenticated user whose
account is in the `example.com` domain.

To allow anyone with an ProjectX account to read items, this line in the relevant
Access file
...
the [ProjectX Security document](/doc/security.md)), which is impractical at best.
In future, ProjectX may provide a mechanism for some sort of key mechanism that
...
One goal of the design of access controls in ProjectX is that a user cannot
easily discover valid names in the ProjectX name space unless granted permission
to do so.
...
If the evaluation of an ProjectX name reaches a link node, the Directory server
...
(These operations are handled automatically by the ProjectX client library.)
...
modified with ProjectX calls to the `DirServer`.
